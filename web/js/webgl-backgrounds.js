...
// ============================================================================
// ANTIGRAVITY THEME STUDIO — OPTIMIZED WEBGL ENGINE (FROM MCP OS)
// Ultra-optimized for minimal GPU & RAM footprint:
// 1. DPR Clamping (1.15x) -> Reduz mais de 60% da carga de fragment shader na GPU Retina
// 2. WEBGL_lose_context -> Liberação imediata de VRAM ao trocar/pausar shaders
// 3. Page Visibility API -> Pausa de renderização quando em segundo plano
// ============================================================================

export const OPTIMIZED_DPR = Math.min(window.devicePixelRatio || 1, 1.15);
...
function createOptimizedRenderer(opts = {}) {
  return new Renderer({
    dpr: OPTIMIZED_DPR,
...
function initStrands(ctn, opts = {}) {
  ...
  const renderer = createOptimizedRenderer({ alpha: true, premultipliedAlpha: true, antialias: true });
...
function initDither(ctn, opts = {}) {
  ...
  const renderer = createOptimizedRenderer({ alpha: true, antialias: true });
  const gl = renderer.gl;
  gl.canvas.style.width = '100%';
  gl.canvas.style.height = '100%';
  ctn.appendChild(gl.canvas);
...
window.stopWebGLBackground = function() {
  ...
  if (currentCleanup) {
    try {
      ...
      console.warn('Erro durante cleanup WebGL:', err);
    }
    currentCleanup = null;
  }
  if (container) {
...
// Catálogo Oficial de Presets para a Interface do Antigravity
window.ANTIGRAVITY_BG_PRESETS = [
  { id: 'softaurora', name: 'Soft Aurora', emoji: '🌌', desc: 'Ondas boreais etéreas em carmesim e rubi profundo' },
  { id: 'balatro', name: 'Balatro Spiral', emoji: '🃏', desc: 'Espiral psicodélica hipnótica com rotação matemática' },
  { id: 'strands', name: 'Strands', emoji: '✨', desc: 'Fios luminosos neon entrelaçados em fluxo contínuo' },
  { id: 'lighttunnel', name: 'Light Tunnel', emoji: '🚀', desc: 'Túnel de fibra óptica com pulso tridimensional' },
  { id: 'siderays', name: 'Side Rays', emoji: '⚡', desc: 'Feixes de luz volumétricos e feixes diagonais' },
  { id: 'plasmawave', name: 'Plasma Wave', emoji: '🌊', desc: 'Ondulações fluidas de plasma eletromagnético' },
  { id: 'ferrofluid', name: 'Ferrofluid', emoji: '🧲', desc: 'Fluido magnético reativo com brilho metálico' },
  { id: 'moltenmetal', name: 'Molten Metal', emoji: '🌋', desc: 'Metal líquido cromado em constante fusão e relevo' },
  { id: 'topography', name: 'Topography', emoji: '🗺️', desc: 'Curvas de nível topográficas em camadas orgânicas' },
  { id: 'darkveil', name: 'Dark Veil', emoji: '🌑', desc: 'Véu escuro sutil com distorções de luz suave' },
  { id: 'dither', name: 'Dither Matrix', emoji: '👾', desc: 'Estética retrô pontilhada em matriz pontual' },
  { id: 'webthreads', name: 'Web Threads', emoji: '🕸️', desc: 'Rede tridimensional de conexões orgânicas e nós' },
  { id: 'acidsquares', name: 'Acid Squares', emoji: '🟩', desc: 'Grade geométrica psicodélica com pulso radial' },
  { id: 'none', name: 'Desativado (Modo Eco)', emoji: '🖤', desc: 'Zero GPU e bateria ilimitada — fundo escuro estático' }
];

window.getCurrentWebGLBackground = function() {
  return currentActiveType || 'none';
};
